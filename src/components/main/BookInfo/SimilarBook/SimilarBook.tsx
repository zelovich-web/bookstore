...
const SimilarBook: React.FC<SimilarBookProps> = ({ bookTitle }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`https://api.itbook.store/1.0/search/${bookTitle}`);
  ...
  return (
    <div className={styles.SliderContainer}>
      <div className={styles.SlliderItems}>
        <h2 className={styles.SliderTitle}>Similar Books</h2>
